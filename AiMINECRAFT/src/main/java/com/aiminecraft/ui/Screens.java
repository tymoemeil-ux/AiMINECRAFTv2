...
public final class Screens {
    ...
    private static void button(Game g, GuiRenderer r, float x, float y, float w, float h, String label) {
        ...
        r.fillRect(x, y, w, h, hover ? BTN_BG_HOVER : BTN_BG);
        r.drawBorder(x, y, w, h, 2, BTN_EDGE);
        r.drawCenteredText(g.font, label, x + w / 2f, y + h / 2f - 8, 1.0f, WHITE);
    }
    ...
    private static void drawTitle(Game g, GuiRenderer r, int w, int h) {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            int cg = (int) (40 + t * 110);
            int cb = (int) (90 + t * 130);
            r.fillRect(0, h * t, w, h / 31f + 1, 0xFF000000 | (cr << 16) | (cg << 8) | cb);
        }
        // "Ziemia" na dole.
        r.fillRect(0, h - 90, w, 26, 0xFF5D9C3A);
        r.fillRect(0, h - 64, w, 64, 0xFF6B5433);
        r.fillRect(0, h - 90, w, 3, 0xFF79C14E);

        r.drawCenteredText(g.font, "AiMINECRAFT", w / 2f, h * 0.22f, 4.0f, WHITE);
        r.drawCenteredText(g.font, "wersja 2.0", w / 2f, h * 0.22f + 70, 1.0f, YELLOW);
        ...
        r.drawCenteredText(g.font, "Sterowanie: WASD + mysz, E - ekwipunek, T - chat, G - panel admina",
                w / 2f, h - 34, 1.0f, GRAY);
    }

...
public final class Screens {
    ...
    private static void drawTitle(Game g, GuiRenderer r, int w, int h) {
        ...
        r.drawTextShadow(g.font, footer, (w - r.textWidth(g.font, footer, 1.0f)) / 2f,
                h - 158, 1.0f, WHITE);
        r.drawCenteredText(g.font, "1 / 2 / 3 - wybor | Enter - Graj | F11 - pelny ekran",
                w / 2f, by + 162, 1.0f, GRAY);
        mouseDebug(g, r, w, h);
    }
    ...
    /** Diagnostyka: wersja + pozycja myszy + marker kursora. */
    private static void mouseDebug(Game g, GuiRenderer r, int w, int h) {
        r.drawText(g.font, Game.VERSION, w - 70, h - 24, 1.0f, GRAY);
        r.drawText(g.font, "mysz: " + (int) g.mouseX + "," + (int) g.mouseY, 10, 10, 1.0f, GRAY);
        r.fillRect(g.mouseX - 8, g.mouseY - 1, 16, 2, RED);
        r.fillRect(g.mouseX - 1, g.mouseY - 8, 2, 16, RED);
    }
    ...
    private static void drawPlayMenu(Game g, GuiRenderer r, int w, int h) {
        ...
        // Zaznaczenie klawiatury (strzalki + Enter).
        float[][] focusRects = {
            {px + 30, py + 236, pw - 60, 36},
            {px + 30, py + 288, bw, 40},
            {px + 45 + bw, py + 288, bw, 40},
            {px + 30, py + 338, bw, 40},
            {px + 45 + bw, py + 338, bw, 40}
        };
        float[] fr = focusRects[g.playMenuFocus];
        r.drawBorder(fr[0] - 3, fr[1] - 3, fr[2] + 6, fr[3] + 6, 2, YELLOW);
        r.drawText(g.font, "Strzalki + Enter (albo mysz)", px + 30, py + ph - 22, 1.0f, GRAY);
        mouseDebug(g, r, w, h);
    }
    ...
    private static void drawConfirmDelete(Game g, GuiRenderer r, int w, int h) {
        ...
        r.drawCenteredText(g.font, "Y - tak | N - nie", w / 2f, py + 78, 1.0f, GRAY);
    ...
    private static void drawSettings(Game g, GuiRenderer r, int w, int h) {
        ...
        r.drawText(g.font, "Strzalki: wybor i zmiana | Enter: Gotowe", px + 30, y + 22, 1.0f, GRAY);
        // Zaznaczenie klawiatury.
        if (g.settingsFocus < 5) {
            float fy = py + 70 + g.settingsFocus * 46;
            r.drawBorder(px + 20, fy - 5, pw - 40, 44, 2, YELLOW);
        } else {
            r.drawBorder(px + 27, py + ph - 63, pw - 54, 46, 2, YELLOW);
        }
    }
    ...
    private static void drawPause(Game g, GuiRenderer r, int w, int h) {
        ...
        r.drawCenteredText(g.font, "Klawisze: 1 / 2 / 3", w / 2f, by + 192, 1.0f, GRAY);
    }
    ...
    private static void drawAdmin(Game g, GuiRenderer r, int w, int h) {
        ...
        for (int i = 0; i < ADMIN_BTNS.length; i++) {
            ...
            button(g, r, bx, by, 220, 40, (i + 1) + ". " + label);
        }

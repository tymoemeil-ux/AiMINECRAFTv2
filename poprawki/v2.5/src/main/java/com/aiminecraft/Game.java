...
public class Game {
    ...
    /** Wersja pokazywana w menu (diagnostyka). */
    public static final String VERSION = "v2.5";
    /** Zaznaczenie klawiatury w menu wyboru swiata (0-4) i ustawieniach (0-5). */
    public int playMenuFocus = 1;
    public int settingsFocus = 0;
    ...
    private boolean f11Prev = false;
    private boolean tabPrev = false;
    private boolean upPrev = false;
    private boolean downPrev = false;
    private boolean leftArrowPrev = false;
    private boolean rightArrowPrev = false;
    private boolean yPrev = false;
    private boolean nPrev = false;
    ...
    private void handleKeys(double dt) {
        ...
        boolean f11 = window.isKeyPressed(GLFW_KEY_F11);
        boolean f11Edge = f11 && !f11Prev;
        f11Prev = f11;
        if (f11Edge) {
            window.toggleFullscreen();
        }

        boolean tab = window.isKeyPressed(GLFW_KEY_TAB);
        boolean tabEdge = tab && !tabPrev;
        tabPrev = tab;
        boolean up = window.isKeyPressed(GLFW_KEY_UP);
        boolean upEdge = up && !upPrev;
        upPrev = up;
        boolean down = window.isKeyPressed(GLFW_KEY_DOWN);
        boolean downEdge = down && !downPrev;
        downPrev = down;
        boolean leftArrow = window.isKeyPressed(GLFW_KEY_LEFT);
        boolean leftArrowEdge = leftArrow && !leftArrowPrev;
        leftArrowPrev = leftArrow;
        boolean rightArrow = window.isKeyPressed(GLFW_KEY_RIGHT);
        boolean rightArrowEdge = rightArrow && !rightArrowPrev;
        rightArrowPrev = rightArrow;
        boolean yKey = window.isKeyPressed(GLFW_KEY_Y);
        boolean yEdge = yKey && !yPrev;
        yPrev = yKey;
        boolean nKey = window.isKeyPressed(GLFW_KEY_N);
        boolean nEdge = nKey && !nPrev;
        nPrev = nKey;
        ...
        switch (state) {
            ...
            case PLAY_MENU:
                if (escEdge) {
                    ...
                    playMenuFocus = (playMenuFocus + 4) % 5;
                    Sound.play("click");
                } else if (downEdge || tabEdge) {
                    playMenuFocus = (playMenuFocus + 1) % 5;
                    Sound.play("click");
                } else if (leftArrowEdge || rightArrowEdge) {
                    if (playMenuFocus == 1) {
                        playMenuFocus = 2;
                    } else if (playMenuFocus == 2) {
                        playMenuFocus = 1;
                    } else if (playMenuFocus == 3) {
                        playMenuFocus = 4;
                    } else if (playMenuFocus == 4) {
                        playMenuFocus = 3;
                    }
                    Sound.play("click");
                } else if (enterEdge) {
                    activatePlayMenuFocus();
                }
                break;
            case CONFIRM_DELETE:
                if (escEdge || nEdge) {
                    uiCancelDelete();
                } else if (enterEdge || yEdge) {
                    uiConfirmDelete();
                }
                break;
            case SETTINGS:
                if (escEdge) {
                    ...
                    settingsFocus = (settingsFocus + 5) % 6;
                    Sound.play("click");
                } else if (downEdge || tabEdge) {
                    settingsFocus = (settingsFocus + 1) % 6;
                    Sound.play("click");
                } else if (leftArrowEdge) {
                    settingsAdjust(-1);
                } else if (rightArrowEdge) {
                    settingsAdjust(1);
                } else if (enterEdge) {
                    if (settingsFocus == 5) {
                        uiSettingsDone();
    ...
    /** Aktywuje zaznaczony przycisk w menu wyboru swiata. */
    private void activatePlayMenuFocus() {
        switch (playMenuFocus) {
            case 0:
                uiToggleNewGamemode();
                break;
            case 1:
    ...
    /** Zmienia zaznaczone ustawienie (kierunek -1 / +1). */
    private void settingsAdjust(int dir) {
        if (settingsFocus < 3) {
            if (dir < 0) {
                uiSetMinus(settingsFocus);
            } else {
                uiSetPlus(settingsFocus);
            }
